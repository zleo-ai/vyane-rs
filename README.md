...
| **MCP server** (`vyane mcp` — nine tools) | `vyane-mcp` + `rmcp` 3.3 | [x] six base tools plus authenticated durable workflow submit/status/cancel; generic success output has a 1 MiB cap; stdio dual-era handshake keeps `initialize` on 2025-11-25 and can negotiate MCP 2026-07-28 over `server/discover` |
