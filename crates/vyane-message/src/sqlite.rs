...
fn connection_lock_key(path: &Path) -> Result<PathBuf> {
    ...
    let Some(file_name) = path.file_name() else {
        return Err(MessageStoreError::InvalidInput(
            "message database path must include a file name".into(),
        ));
    };
...
mod tests {
    ...
    fn aliased_paths_share_the_process_connection_lock() {
        ...
        let a = SqliteMessageStore::open(&direct).unwrap();
        let b = SqliteMessageStore::open(&dotted).unwrap();
        let c = SqliteMessageStore::open(&via_parent).unwrap();
        let d = SqliteMessageStore::open(&other).unwrap();
