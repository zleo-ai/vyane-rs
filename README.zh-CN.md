...
| owner-scoped goal 生命周期与进度真相源 | `vyane-goal` | [~] 同一 SQLite 事务更新当前快照并追加不可变事件；WP-68 增加有界本地 acceptance verifier，WP-69 保存不可变 evidence，WP-70 通过 fresh production dispatch segment 增加显式有界 manual pursuit，WP-71 为该手动循环增加持久、lease-fenced 的重启 checkpoint；daemon 自动 pursuit、approval、quota handoff 与 authenticated goal service 仍待后续层 |
...
它不会静默 start goal、不会在重启后自动选择或 pursuit goal、不会恢复 runtime-native session，
也不做 quota/approval handoff。
生产 CLI 当前只接受与 dispatch ledger 一致的 `local` single-user owner，避免 goal 与执行
authority 静默分叉。见 [WP-70](docs/plan/WP-70.md) 与 [WP-71](docs/plan/WP-71.md)。
