...
| owner-scoped goal 生命周期与进度真相源 | `vyane-goal` | [~] 同一 SQLite 事务更新当前快照并追加不可变事件；WP-68 增加有界本地 acceptance verifier，WP-69 保存不可变 evidence，WP-70 通过 fresh production dispatch segment 增加显式有界 manual pursuit；自动/可恢复 pursuit、approval、quota handoff 与 authenticated goal service 仍待后续层 |
