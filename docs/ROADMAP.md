...
[WP-449](plan/WP-449.md) adds pure allowlisted operator-command lines on REST
continuity-next success when a command is projected.
...
[WP-454](plan/WP-454.md) reuses pure continuity signal-kind lines on CLI
continuity-execute when ready signals are assembled into prompt evidence.
...
[WP-459](plan/WP-459.md) reuses pure takeover-sandbox lines on CLI
continuity-execute for the frozen sandbox before dispatch.
