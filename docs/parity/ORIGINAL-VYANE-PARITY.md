...
| GOV-03 | sandbox / filesystem capability gate | `partial` | public-core | coding harness flag、native OS sandbox 与隔离 transport 共同承载 filesystem authority。 | Kernel 已在任何 `make`/HTTP/subprocess 前完成整链 trusted `CapabilityManifest` admission；Linux mutating path 以 open-first fd/identity、descriptor-backed cwd 与 `/proc/self/fd` 抵抗路径替换，非 Linux fail closed。WP-83A 的只读 native builtins 现在从 exact `PinnedWorkdir` 逐分量 `O_NOFOLLOW` 打开，拒绝 absolute/parent traversal 与 symlink，并在每个 open/enumeration 重验 live native authority；root rename/replacement 与 revocation regression 已覆盖。admitted workspace 默认广泛可读，submission 可把 glob exclusions 冻结进 policy digest，同时约束 read/search。该窄 path capability 不是 child-process OS sandbox，同 UID child、命令、网络与 `Full` 仍不受 host confinement。 | 保持 admission/pin/native-scope regression；将 user/project/managed permission layers 解析为不可放宽 managed floor 的 per-run frozen policy。为 write/edit 增加 open/publish 双重验权与 atomic drift-safe publication，再为 command 建立 fail-closed host-enforced child sandbox。不得把 descriptor-relative trusted I/O、`AdapterDelegated` 或 pinned cwd 描述成 same-UID confinement/microVM。 | P1 |
