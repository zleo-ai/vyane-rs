...
| CON-07 | GoalStore、acceptance、quota handoff、pursuit | `partial` | decision | goal lifecycle 包含 acceptance、progress、quota handoff 与自动 pursuit。 | WP-60 新增 `vyane-goal`：owner-scoped 单一 SQLite 真相源、事务型 snapshot/event revision、六态 lifecycle、priority queue、progress、acceptance descriptor，以及本地 `vyane goal` CLI。WP-68 增加 model-free bounded local verifier；WP-69 以 schema v3 保存 owner-scoped、immutable、digest-checked 的完整 bounded verification artifact。WP-70 增加显式 manual pursuer：在 owner/lease fence 下执行 verify→fresh production dispatch segment→reverify，只有全部 acceptance 满足时由 pursuer 完成；manual/no-criteria/segment/failure/timeout 会 pause，外部 lifecycle 变化会 stop。仍无 daemon automatic/resumable pursuit、quota handoff/continuity、authenticated service/API、approval/kernel takeover 或网络/board verifier。 | 保持 ledger 与 pursuer 分层；继续定义 quota continuity、approval/kernel takeover/review/handback 与 restart authority，不能因 manual loop 已存在而宣称完整自动 pursuit。 | P2 |
