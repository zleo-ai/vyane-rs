...
| CON-07 | GoalStore、acceptance、quota handoff、pursuit | `partial` | decision | goal lifecycle 包含 acceptance、progress、quota handoff 与自动 pursuit。 | WP-60 新增 `vyane-goal`：owner-scoped 单一 SQLite 真相源、事务型 snapshot/event revision、六态 lifecycle、priority queue、progress、acceptance descriptor，以及本地 `vyane goal` CLI。WP-68 增加 model-free bounded local verifier：显式 `cmd:`、canonical workdir、scrubbed env、timeout/output cap、Unix process-group cleanup，并只通过已有 lease-fenced mutation 持久化通过项。WP-69 以 schema v3 保存 owner-scoped、immutable、digest-checked 的完整 bounded verification artifact，并在 criterion mutation 前先落证据。仍无自动 pursuer、quota handoff/continuity、authenticated service/API、approval/kernel takeover 或网络/board verifier。 | 保持 ledger 与 pursuer 分层；继续定义明确 completion authority、approval/kernel takeover/review/handback，不能因本地 verifier 与 artifact 已存在而自动完成 goal。 | P2 |
