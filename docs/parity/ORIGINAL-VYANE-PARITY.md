...
| COL-03 | durable local AgentRun queue/inbox | `partial` | public-core | owner-aware durable queue 支持 delayed delivery、claim/read 与 worker inbox。 | `vyane-agent` 已有 owner-scoped FIFO/generation/lease/recovery/resume/topology/cancel/outbox；`vyane-message` 与 broker 分离持有 message/delivery。WP-59 新增同机 `a2a send/inbox/read`。WP-61 与 WP-63 至 WP-66 将 generic execution/recovery/publication host、private frozen spool、exact Linux Process sidecar、fresh/sessionless Native lane 与 message completion handback 生产 assembly 到 authenticated loopback daemon AgentRun submit/status/output/cancel；仅支持 fixed local owner，restart 不 replay。仍无 `Remote`、domain-aware native resume、principal-derived message API 或 Channels adapter | 保持 fencing/recovery/execution/publication ordering、identity reuse、Native daemon-level graceful-shutdown race tests；下一步把 fixed local bearer 升级为 principal-derived scope，并接 `Remote`/protocol adapter。remote send→local receipt 不得宣称 distributed exactly-once。 | P2 |
...
| INT-04 | resident daemon | `different` | decision | 广义 daemon 同时承载 worker、scheduler、event、message、memory、goal 与 channel；workflow HTTP 属于另一 process surface。 | Rust daemon 已持有 authenticated local workflow execution，并以 WP-61 与 WP-63 至 WP-66 production-assemble 共享 AgentRun execution/recovery/completion host、fresh/sessionless CLI-harness Process lane 与 fresh/sessionless Native lane；两条 lane 具备 exact backend recovery/cancel 与 message handback，启动先 recovery，restart 不 replay。Process cooperative drain 已有 acceptance，Native daemon-level graceful-shutdown race acceptance 仍未收口。broker supervisor 已组装但 delivery lane 刻意为空；仍没有 `Remote`、native session resume、distinct-principal wiring、live pause/resume 或 automatic replay。ADR 0004 接受由窄 domain supervisor 组成 authenticated resident host | 补 Native graceful-shutdown race acceptance，继续接入 concrete broker delivery lane 与 `Remote`，统一 principal-derived owner context 和 bounded drain；用 executable acceptance 分别证明每个 domain，不得因已有窄 Process/Native host 或同名 `daemon` 宣称广义 daemon parity。 | P0 |
...
exact backend recovery/cancel 与 message completion handback；Process cooperative shutdown 已有
acceptance，Native daemon-level graceful-shutdown race acceptance 仍未收口。resident broker supervisor
也已生产组装，但 delivery lane set 刻意为空。下一步是补该 Native shutdown acceptance、concrete broker
delivery adapter、`Remote`、session-aware native resume、distinct-principal owner context、公开 CLI 与 A2A/Channels adapter。
