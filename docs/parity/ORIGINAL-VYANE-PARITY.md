...
| EXE-09 | durable detached tasks 与 resident workflow execution | `different` | public-core | async dispatch 依赖进程内 task 与状态文件，另有更广义的 resident worker runtime。 | `vyane-rs:crates/vyane-task`; `crates/vyane-cli/src/{api,command,daemon,daemon_workflow,workflow_control}.rs`; `crates/vyane-cli/src/task/{mod,proc,spawn,store}.rs`；新 CLI detached submission 在 task row/process 前完成 capability/session admission，冻结 secret-free plan，并在 Linux 把同一 pinned directory fd 交给 worker 复核；Rust 在 metadata/CAS/controller recovery 上更强 | 保留 Rust 强契约；补迁移说明并验证 client exit 后继续、exact cancel、restart→interrupted、不泄露 payload，以及 parent rejection 无 task/process/HTTP、worker plan/identity drift fail closed。旧 `job.json` compatibility path 不得被描述成所有任务都有新 fd handoff。 | — |
...
| CON-06 | worker health + session-aware auto-resume | `partial` | optional-adapter | worker health scanner 可识别 controller loss，并在有限条件下尝试 session-aware recovery。 | `vyane-agent` 已提供 durable lease/deadline、heartbeat、two-stage recovery、resume admission 与 active permit。WP-45/47/48 提供 one-shot recovery/execution 与 exact paired `InProcess` backend；exact fingerprint、`Notify`、bounded/reclaimable tombstone、late-register 及 per-effect permit revalidation 关闭 replacement race。WP-51 将该 pairing 消费进 resident execution/recovery loop；WP-53 增加 body-free completion receipt、隐藏 message staging、exact lease-loss 对账与独立 publication loop。degraded/error/panic 有界退避；host cancel 仅停止新 cycle，已开始 pass drain，且从不自动 enqueue resume。它仍没有 concrete product operation、Process/Remote、session-aware production authority、protocol/host assembly 或非文件系统 fencing | 将 concrete product integration 接入 modular host；仅在 exact native session domain、policy allow、controller gone proof、active permit revalidation、live session lease 和 bounded retry 下恢复。 | P2 |
| CON-07 | GoalStore、acceptance、quota handoff、pursuit | `missing` | decision | goal lifecycle 包含 acceptance、progress、quota handoff 与自动 pursuit。 | Rust 无 goal crate/CLI/API | 先区分 durable goal ledger 与自动 pursuer；实现 create/lifecycle/progress/acceptance 后，再经 approval/kernel 接 takeover/review/handback。 | P2 |
...
| COL-03 | durable local AgentRun queue/inbox | `partial` | public-core | owner-aware durable queue 支持 delayed delivery、claim/read 与 worker inbox。 | `vyane-agent` 已有 owner-scoped FIFO/generation/lease/recovery/resume/topology/cancel/outbox；`vyane-message` 与 broker 分离持有 message/delivery。service 的 paired `InProcess` backend 固定 owner/store/operation；WP-53 以 body-free descriptor、hidden stage、recovery reconciliation 与 replay-safe publisher 提供 generic local result handback。WP-55 又为 Claude/Codex capture/streaming 增加 process-local spawn authority，在 wrapper spawn 与 gated target release 前重验，并隔离 sentinel/target env；但尚无 AgentRun 生产 caller 构造该 authority。仍无 exact process sidecar、concrete Process/Remote、production host、domain-aware resume、worker input、公开 message API、A2A 或 Channels adapter | 保持现有 fencing/recovery/execution/publication ordering 与 race tests；先以 owner-bound exact process identity/sidecar 把 live gate 接入 Process backend，再接 production host 与 public APIs。remote send→local receipt 不得宣称 distributed exactly-once。 | P2 |
| COL-04 | A2A HTTP v0.3、Agent Card、SSE、push | `missing` | optional-adapter | A2A server/client 覆盖 Agent Card、request/subscribe/cancel 与 push delivery。 | Rust 无 A2A server/client surface | 用协议 fixture 验证 Agent Card、send/get/cancel/sendSubscribe、Bearer owner binding 和 SSRF-safe push；作为可选 crate。 | P3 |
| COL-05 | worker/child topology、tree cancel、worker messaging | `partial` | public-core | worker runtime 支持 child spawn、tree cancel 与 worker-to-worker message。 | `vyane-agent` 已实现 immutable parent、fenced child spawn、bounded topology、children-first tree cancel 与 frozen operation scope。WP-51 resident library supervisor 只组合 paired execution/recovery，不消费 topology/tree cancel/message，且没有 concrete operation 或 message input/handback。仍无真实 Process/Remote cancel/recovery、tree-cancel consumer 或 service/CLI/API | 把 topology/tree cancel 接入 concrete integrations；用 `vyane-message` 建 exact correlation/handback，不得把 prompt/message 塞进 task/AgentRun row；补 public API 和 crash/retry E2E。 | P2 |
...
| LED-06 | goal ledger | `missing` | decision | goal 使用 append-only truth 与查询索引组合持久化。 | Rust 无 goal store | 与 CON-07 同一工作包；明确单一 truth，不能重复原仓 JSONL/SQLite 多事实源债务。 | P2 |
...
| QUA-04 | 跨实现 golden/shadow parity harness | `partial` | public-core | 参考基线现包含 automatic workflow routing 的 provider/model/effort precedence 行为，但旧 embedded parity bridge/fixture 仍未覆盖当前独立 Rust 实现的完整能力面，也不能把 staged frontier/model policy 冒充为已闭环行为。 | `vyane-rs:docs/parity/fixtures/v1` 与 `crates/vyane-cli/tests/parity_manifest.rs` 只公开 maintainer-attested、已净化的 classifier/failover 行为 fixture，以公开文件 SHA-256、closed disposition、case digest 和当前 Rust 重算锁定；不发布私有 repository、commit、blob 或 source path provenance。精选 15 case 中 13 normalized exact、2 open difference。现有两套行为 fixture 共 161 case，但尚无 automatic target、显式 precedence、full-chain frontier guard、target resolve、production failover trail、run schema、session、workflow migration或 refresh/diff report 的跨实现 fixture | 扩展同一 manifest/normalizer 到净化的 automatic routing case：显式 provider/model/effort 优先级、rendered workflow prompt、no-eligible fail-closed、full-chain frontier guard 与 frozen detached/replay route；再覆盖剩余 routing/failover、target resolve、run/session schema、workflow migration并增加可复现 refresh/diff report。fixture 只使用中性 profile/model id，open difference 只能经 ADR/migration evidence 关闭。 | P0 |
...
| OBS-06 | health、watchdogs、quota ledger、notifications | `missing` | optional-adapter | daemon health、worker/session watchdog 与 notification store 形成运维信号；新的 upstream quota/balance 能力是对有限 connector 集合的并发 snapshot view，将 absolute/balance/window 信号归一化并隔离单 connector 失败。它当前不是 durable history/ledger，不能用名称把一次快照扩写成可回放账本。 | Rust 只有 serve/daemon health 与 tracing；`vyane-ledger` 的 run usage/cost 估算不是 upstream quota snapshot，CLI 也无对应 connector surface | 先定义 platform-neutral `QuotaConnector` 和 closed status/window/card schema，实现有界并发、timeout/body cap/redirect policy、稳定排序、单 connector 失败隔离与 redacted error；公开验收只用 fixture/fake transport，默认零真实网络，凭据只以 opaque read-only reference 注入。具体平台 watcher/connector 可选，所有自动动作必须走 governance/approval。 | P3 |

### 4.8 Interfaces

| ID | 能力 | 状态 | 目标层 | 参考能力/行为（私有基线审计） | Rust 证据/缺口 | 验收条件 | 波次 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| INT-01 | CLI | `partial` | public-core | CLI 覆盖 execution、daemon、collaboration、message、session、goal 与 maintenance 等产品面。 | `vyane-rs:crates/vyane-cli/src/cli.rs#Command` 覆盖 core execution/task/workflow/daemon，并新增 owner-local `session list/inspect/reset-native --expected-revision`；legacy `sessions` 保持兼容 | 每个纳入 public 范围的 service 都有 JSON-stable CLI；private-only 命令不要求复制。建立 `--help` 和 exit-code snapshots。 | P2 |
| INT-02 | REST API | `partial` | public-core | API 覆盖更广的 daemon/product resources，dispatch/workflow SSE 使用 owner-scoped control。 | Rust `serve` 有 core dispatch/broadcast/tasks/runs/sessions，使用 per-start bearer、loopback Host/Origin enforcement 与 allowlisted views。router assembly 将 dispatch/broadcast/runs/sessions/SSE 全部冻结到同一 local `OwnerScopedService`，task truth/artifact 已 owner-qualified；但 bearer 仍不是 distinct principal，未通过 `OwnerContextFactory` 认证。无 hostile same-UID/multi-user isolation；workflow daemon 另有独立 authenticated local API | 定义 API scope/version/auth ADR；把 authenticated request credential 接入 fixed `OwnerContextFactory`，让所有 control surface 使用其 scope，并补 error schema、pagination/idempotency；private resource 走 optional adapter。 | P2 |
| INT-03 | MCP server | `partial` | public-core | 参考 MCP surface 有 19 个工具，覆盖 task control、workflow、board、check 与 collaboration。 | `vyane-rs:crates/vyane-mcp/src/lib.rs` 现有 6 个真实 rmcp/stdio 工具：dispatch/broadcast/history/sessions 加有界 `vyane_route` 与 static-only `vyane_check`。所有 success result 有 generic 1 MiB 输出上限，route/check diagnostics 另有更小上限；旧 execution tool 输入字段尚无 diagnostics 那样统一的逐字段预算。执行已完成但 detail 超限时，dispatch/broadcast 返回 `operation_status=completed` 的有界 run receipt，避免误导重试。`vyane_route` 是 Rust 确定性预览扩展，不是参考系统同名工具等价；`vyane_check` 只做静态 config/assembler/credential-presence/URL validation，不联网、不 probe、不 spawn，也不验证真实 credential。run/session 输出使用 allowlisted view，sessions 语义仍不等于参考系统的多动作 session tool | 经 authenticated `WorkflowControl` port 接 workflow/task；collaboration/board 随对应 core；补 owner context、cancel 与对应 service acceptance，并保持 route/check 的严格 schema、有界/redacted error 和真实 MCP 协议测试。 | P1 |
| INT-04 | resident daemon | `different` | decision | 广义 daemon 同时承载 worker、scheduler、event、message、memory、goal 与 channel；workflow HTTP 属于另一 process surface。 | Rust daemon 只持有 authenticated local workflow execution；broker driver 仍 unwired。WP-51/WP-53 已有 dark paired `InProcess` resident execution/recovery/completion-publication library supervisor 与 generic crash-consistent local message handback，REST service paths 也统一冻结为 local scope。但没有 concrete product operation、Process/Remote、distinct-principal wiring、tree-cancel consumer 或 production host。ADR 0004 接受由窄 domain supervisor 组成 authenticated resident host | production-assemble broker/AgentRun supervisors、concrete integrations、统一 authenticated owner context 和 bounded drain；在 executable host acceptance 前不得因 dark library loops、owner-qualified storage、ADR 或同名 `daemon` 宣称 parity。 | P0 |
| INT-05 | A2A server/client | `missing` | optional-adapter | A2A interface 同时提供 server、client、SSE 与 remote adapter。 | Rust 无 A2A interface | 与 COL-04 同一验收，不另造 task truth。 | P3 |
...
| INT-07 | packaging / registry publication | `different` | decision | 参考系统使用 Python packaging；其分发生命周期与 Rust registry 不同。 | Rust 16 crates 已通过本地 package+verify preflight，含 `vyane-agent` 与显式依赖顺序，但没有发布。WP-56 移除 tag-push 触发：只允许从当前 `main` 手动输入既有 release tag；workflow SHA、`origin/main` 与 tag 必须完全一致，且 `crates-io` environment 必须配置 required reviewers 与 prevent-self-review。registry token 只进入最终 publish step | tag 与 token 都不是授权；必须另获明确外部发布授权，由非发起 reviewer 批准 protected environment，且不得改变任何 parity 行状态。 | — |
...
`crates.io` 是 Rust 社区的公共包注册表，作用类似 Python 的 PyPI。把 16 个 crate 发布到
crates.io，只能证明：
...
1. v1 manifest 已固定精选 classifier/failover case 与 open difference；下一版先补净化的
   automatic workflow route precedence、no-eligible fail-closed、full-chain frontier guard 与 frozen route replay，
   再扩到 EXE-01/02、COL-01、LED-01、完整 oracle case、run/session schema 和 workflow migration。
...
4. 当前 workspace test 已执行 manifest schema/provenance/normalizer 重算；增加可复现 refresh/diff
   report 并保持不要求真实 model key。
...
principal-derived owner service/task truth 已落地；下一步是 broker/AgentRun driver 的 production host assembly、
concrete product operation/controller 与 crash-consistent message handback glue、公开
service/CLI API 与 A2A/Channels adapter。任何 cancel/intervention/timeline mutation 都必须使用
