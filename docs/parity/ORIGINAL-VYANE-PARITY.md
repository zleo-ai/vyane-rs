...
| GOV-03 | sandbox / filesystem capability gate | `partial` | public-core | coding harness flag、native OS sandbox 与隔离 transport 共同承载 filesystem authority。 | Kernel 已在任何 `make`/HTTP/subprocess 前完成整链 trusted `CapabilityManifest` admission；Linux mutating path 以 open-first fd/identity、descriptor-backed cwd 与 `/proc/self/fd` 抵抗路径替换，非 Linux fail closed。WP-83 的 native read/search/write/edit builtins 从 exact `PinnedWorkdir` 逐分量 `O_NOFOLLOW` 打开，拒绝 absolute/parent traversal 与 symlink，并在每个 open/enumeration/publish 重验 live authority；write/edit 使用 descriptor-relative temp publication 与 drift checks。WP-84 的 structured command 在 Linux Bubblewrap 只读 workspace 中运行；WP-90 仅以 descriptor-bound roots 增加显式 writable bind。WP-85 的 command HTTPS egress 经过隔离 network namespace 内的 trusted proxy 与 frozen destination policy，WP-86/87 的 hosted search/direct fetch 则走各自独立 policy。WP-88/91 将 request 与 user/project/explicit/managed ceiling 冻结成不可被配置放宽的 per-run policy。该组合只约束这些 trusted Native tools，不是 CLI harness、任意同 UID child、`Full` 或 microVM confinement。 | 保持 admission/pin/native-scope、open/publish/spawn/network 线性化点与配置单调收紧 regression；为其余 child-process/`Full` execution 明确 host-enforced sandbox 或继续 fail closed。不得把 descriptor-relative trusted I/O、`AdapterDelegated`、pinned cwd 或 Bubblewrap command profile 扩写成整个 daemon 的 same-UID confinement/microVM。 | P1 |
