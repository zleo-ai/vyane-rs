...
| EXE-07 | native self-built harness | `partial` | public-core | 原生 harness 参考能力覆盖 model loop、工具/权限、checkpoint、session continuity 与分阶段 rollout。 | `vyane-rs:crates/vyane-core/src/{native_authority,traits,tool_chat,web_search,web_fetch}.rs`; `crates/vyane-agent/src/{model,store,sqlite}.rs`; `crates/vyane-harness/src/native/{tools,permissions,turn_driver,trusted_files,trusted_commands,trusted_network,trusted_web_search,trusted_web_fetch}.rs`; `crates/vyane-protocol/src/{openai_chat,openai_responses,http,web_fetch}.rs`; `crates/vyane-cli/src/native_agent.rs`：已有 bounded typed model envelope、validated tool registry、permission seam、early scope/admission、Linux pinned workdir、strict `NativeSessionDomain` storage 与本地 `SessionExecutionLease`。WP-41/42/43 提供 live authority、fresh-sessionless bridge 与严格串行 turn driver；WP-66 将 fresh/sessionless native AgentRun lane 组装进 resident host；WP-83/84/85/86/87 接入 descriptor-relative file tools、只读 Bubblewrap command、独立 command HTTPS destination policy、独立 hosted web search target/domain policy 与受限 direct HTTPS text fetch，并由真实 HTTP→tool→durable completion验收。WP-88 增加 user/project/explicit/managed 四类独立权限上限；配置只能收紧显式请求，最终权限在 workdir probe 与模型调用前冻结。WP-90 在保持完整 workspace 只读的基础上，以 `openat2` 钉住并通过 Bubblewrap `--bind-fd` 单独开放显式 writable roots，配置上限、outer sandbox 与 spool schema/digest 同时约束该权限。WP-91 再把 request 与每个 config ceiling 的有界 allow/ask/deny 规则冻结进 spool；层内 find-last，跨层 deny > ask > allow，allow 不能注册未启用工具。它仍不是通用 `Harness`：无 whole-workspace command write、checkpoint/session commit、approval resume 或 native resume。legacy-unbound/bound state 仍在 `make` 前拒绝，streaming 对任何 session 在 load/probe/`make` 前拒绝；当前 lease 仍是单机 fence，post-model commit 仍 best-effort | 保持每个实际 argument 和 open/publish/spawn/network 线性化点重验；再建立 session-aware authority 组合 exact domain/lease、checkpoint、revision-fenced commit 与 approval store，通过 drift/cancel/retry E2E 后才启用 native resume。非文件系统 store 另补 generation/TTL/stale-holder fencing；随后补容错 tool-call、事件，以及 hooks、skills/MCP、compaction、task subagent、frozen memory。 | P1 |
...
| EXE-09 | durable detached tasks 与 resident workflow execution | `different` | public-core | async dispatch 依赖进程内 task 与状态文件，另有更广义的 resident worker runtime。 | `vyane-rs:crates/vyane-task`; `crates/vyane-cli/src/{api,command,daemon,daemon_workflow,workflow_control,daemon_agent,agent_host,agent_process,agent_spool,native_agent,native_agent_spool}.rs`; `crates/vyane-cli/src/task/{mod,proc,spawn,store}.rs`；CLI detached submission 在 task row/process 前完成 capability/session admission，冻结 secret-free plan，并在 Linux 把同一 pinned directory fd 交给 worker 复核。WP-61 在 daemon 中生产 assembly fresh/sessionless CLI-harness Process AgentRun；WP-63 至 WP-66 增加 durable backend routing、共享 recovery/completion host 与 fresh/sessionless Native lane。两条 lane 都冻结 private spool/authority，支持 exact recovery/cancel 与 message-store completion handback；restart 不 replay。Native daemon-level graceful-shutdown race 已覆盖 active wire cooperative cancellation、不确定输入保留、deadline 后 restart settlement、no replay/no output 与终态 exact residue cleanup。WP-62 只处理 MCP workflow control，不属于该执行链。Rust 在 metadata/CAS/controller recovery 上更强 | 保留 Rust 强契约；继续验证 client exit、exact cancel、restart no-replay、payload 不泄漏与 identity drift fail closed，并保留 Native daemon-level graceful-shutdown race 回归。旧 `job.json` compatibility path 不得被描述成所有任务都有新 fd handoff；现有 Process/Native lane 也不得扩写成 `Remote`、session-aware host 或广义 worker runtime。 | — |
...
| CON-07 | GoalStore、acceptance、quota handoff、pursuit | `partial` | decision | goal lifecycle 包含 acceptance、progress、quota handoff 与自动 pursuit。 | WP-60 新增 `vyane-goal`：owner-scoped 单一 SQLite 真相源、事务型 snapshot/event revision、六态 lifecycle、priority queue、progress、acceptance descriptor，以及本地 `vyane goal` CLI。WP-68 增加 model-free bounded local verifier；WP-69 以 schema v3 保存 owner-scoped、immutable、digest-checked 的完整 bounded verification artifact。WP-70 增加显式 manual pursuer：在 owner/lease fence 下执行 verify→fresh production dispatch segment→reverify，只有全部 acceptance 满足时由 pursuer完成；manual/no-criteria/segment/failure/timeout 会 pause，外部 lifecycle 变化会 stop。WP-71 以 schema v4 增加 digest-checked pursuit checkpoint，并用 goal revision、checkpoint revision 与 lease generation fence 将 checkpoint/progress 原子提交；显式 resume+new claim 可在 DB 重开后继续 lifetime counters，旧 worker fail closed。WP-72 在显式 target/workdir/sandbox opt-in 下把同一 pursuer接入常驻 daemon：一次处理一个 local goal，先续接 eligible in-progress work、再原子 claim queued goal；稳定 worker lease 支持 daemon replacement 续接，paused 与 live foreign lease 不会自动 resume 或被窃取，shutdown 保留 running checkpoint。WP-73 以 schema v6 加入分轴 typed continuity policy，并把规范化 quota event 幂等写入可见 handoff plan；无 takeover 明确记录 manual blocker，有 takeover 只生成 declaration，不启动执行。WP-74 以 schema v7 增加 owner-scoped、严格绑定且一次性消费的 takeover approval/dispatch。WP-75 以 schema v8 在 takeover 成功后释放 review step，将 review approval 绑定 exact takeover approval/run evidence，并在 review 成功后只解除 review dependency，primary 仍显式等待 quota reset。WP-76 增加 `goal continuity-signal ... quota-reset`：精确绑定当前 quota event 与 primary provider/harness/model，原子保存 typed readiness fact；review 与 reset 任意顺序到达都只在两者齐备时将 `resume_primary` 标为 ready，且不消费 approval、不 dispatch。WP-77 让 ready 的 `resume_primary` 复用现有 durable approval/decision/one-shot dispatch：需要 review 时绑定 exact successful review run 并递归校验 review→takeover chain，prompt 带 approved target axes、quota reset 与 handback evidence；consume/finish 与 visible in-flight/done/blocked step transition 原子配对。成功仍不完成 goal，不满足 acceptance，且是 fresh continuation segment，不等于 runtime-native session resume。WP-78 增加 opt-in review-check gate：typed passed/failed signal 绑定当前 quota boundary、公开 repo/PR 坐标、stable observation ID 与 monotonic sequence；旧观察重放不改变 gate，failed 后即使 later passed 也必须等待 approval-gated repair 成功，重复 failure 会重新打开 repair，resume approval 绑定最新 exact repair run 并递归校验 repair→review→takeover chain。WP-79 增加只读、revision-bound 的 operator next-action 投影，区分 queue/decide/execute/signal/wait/blocked/complete 且不执行任何动作。WP-80 只把这一个投影暴露为 opt-in、固定 `local` owner、bearer-authenticated loopback GET：goal 与 approvals 在同一只读事务中形成 allowlisted redacted DTO，request 不能选择 owner。WP-81 另建 owner-frozen/source-bound mutation port，把 quota reset 与 sequenced review-check typed observation 映射到同一 `record_continuity_signal` 事务；one-shot poller 限制 watcher 数、并发、timeout 与 batch，fetch 失败隔离，超限 batch 在任何 item 写入前整体拒绝。WP-82 增加 purpose-separated authentication 与 one-shot bounded runner：有限 goal 集只消费闭合 projection，queue/approved-execution port 仅接收 exact revision/quota/step/approval fence；没有 decision port，人工 approve/reject 与 durable consume/finish fence 不可绕过。runner 没有 transport、concrete port、retry、周期 loop、goal completion 或 runtime-native resume。 | 保持 quota reader、observation ingress、runner、pursuer、continuity state 与 daemon assembly 分层；生产 queue/execution adapter 必须重读 exact fence 并复用现有 durable transaction，不能把 WP-80 read bearer 当 authority。之后再实现真正 session-aware resume，不能把 fresh continuation扩写成 native session 已恢复。 | P2 |
...
| COL-03 | durable local AgentRun queue/inbox | `partial` | public-core | owner-aware durable queue 支持 delayed delivery、claim/read 与 worker inbox。 | `vyane-agent` 已有 owner-scoped FIFO/generation/lease/recovery/resume/topology/cancel/outbox；`vyane-message` 与 broker 分离持有 message/delivery。WP-59 新增同机 `a2a send/inbox/read`。WP-61 与 WP-63 至 WP-66 将 generic execution/recovery/publication host、private frozen spool、exact Linux Process sidecar、fresh/sessionless Native lane 与 message completion handback 生产 assembly 到 authenticated loopback daemon AgentRun submit/status/output/cancel；仅支持 fixed local owner，restart 不 replay。Native lane 已有 daemon graceful-stop、deadline restart settlement、no replay 与 exact residue cleanup 回归。仍无 `Remote`、domain-aware native resume、principal-derived message API 或 Channels adapter | 保持 fencing/recovery/execution/publication ordering、identity reuse 与 Native daemon-level graceful-shutdown race tests；下一步把 fixed local bearer 升级为 principal-derived scope，并接 `Remote`/protocol adapter。remote send→local receipt 不得宣称 distributed exactly-once。 | P2 |
...
| LED-05 | approvals/interventions/notifications/artifacts | `partial` | public-core | 独立 typed store 管理 approval、intervention、notification 与 artifact registry。 | `vyane-core::receipt` 提供公开 schema：`CompletionReceipt`、gate（含 truth_probe）、immutable artifact digest、approval gate 记录与 revision-fenced `MemoryReceiptLedger`；`vyane-service::dogfood` hermetic 路径证明 broken baseline 先 fail 再 complete。`KernelStore` 已有 owner-scoped approval decision + CAS；边界 `DecideApproval`/`DenyApproval` 在 resolvable store + pending ask + binding 时写入该 store，无 store 则 fail closed。仍无独立 approval/intervention/notification 产品 store、expiry/resume binding 或生产级 multi-process receipt API | 保持 receipt 不能在 truth 未过时 complete；补 owner-scoped durable approval store、notification、intervention CAS 与 API mutation 审计；payload 与 secret 分离。 | P2 |
...
| GOV-03 | sandbox / filesystem capability gate | `partial` | public-core | coding harness flag、native OS sandbox 与隔离 transport 共同承载 filesystem authority。 | Kernel 已在任何 `make`/HTTP/subprocess 前完成整链 trusted `CapabilityManifest` admission；Linux mutating path 以 open-first fd/identity、descriptor-backed cwd 与 `/proc/self/fd` 抵抗路径替换，非 Linux fail closed。WP-83 的 native read/search/write/edit builtins 从 exact `PinnedWorkdir` 逐分量 `O_NOFOLLOW` 打开，拒绝 absolute/parent traversal 与 symlink，并在每个 open/enumeration/publish 重验 live authority；write/edit 使用 descriptor-relative temp publication 与 drift checks。WP-84 的 structured command 在 Linux Bubblewrap 只读 workspace 中运行；WP-90 仅以 descriptor-bound roots 增加显式 writable bind。WP-85 的 command HTTPS egress 经过隔离 network namespace 内的 trusted proxy 与 frozen destination policy，WP-86/87 的 hosted search/direct fetch 则走各自独立 policy。WP-88/91 将 request 与 user/project/explicit/managed ceiling 冻结成不可被配置放宽的 per-run policy。WP-93 对包含任一 CLI-harness leg 的整链增加独立、单调收紧的 common sandbox ceiling，并在 REST/workflow task identity、journal、dispatch persistence、executor construction 与 spawn 前拒绝超限请求；vendor flags 仍由 adapter 封闭映射。该 ceiling 只做准入，不会把 `AdapterDelegated` 变成 host-enforced sandbox，也不改变 direct HTTP 或 Native/Canto 权限路径。 | 保持 admission/pin/native-scope、open/publish/spawn/network 线性化点与配置单调收紧 regression；为其余 child-process/`Full` execution 明确 host-enforced sandbox 或继续 fail closed。不得把 descriptor-relative trusted I/O、`AdapterDelegated`、pinned cwd、CLI permission flags 或 Bubblewrap command profile扩写成整个 daemon 的 same-UID confinement/microVM。 | P1 |
...
| GOV-05 | allow/deny/ask permission + approval binding | `partial` | public-core | native permission 层支持 ordered allow/deny/ask 与 approval binding。 | `vyane-rs:crates/vyane-harness/src/native/{permissions,tools,turn_driver,trusted_files}.rs` 已有 ordered allow/ask/deny、default deny、protected deny floor、risky-operation ask、canonical plan hash + call binding；resident native lane 只走 authorized model/tool entry。WP-83A trusted read/search 会校验 exact argument set、路径与界限，并在 registry gate 之外对每个 open/enumeration 再验 authority；兼容无 authority entry 明确 fail closed。WP-91 增加 closed/bounded tool-policy schema：request 与 user/project/explicit/managed 配置逐层保留，层内 find-last，跨层取 deny > ask > allow，并随 spool policy digest 冻结；真实 resident E2E 证明 config ask 在写入和模型重放前停止。Allow 只表示该层不再收紧，不能启用 absent tool 或覆盖更严格层。Ask 仍是持有 exact plan 的 typed non-replayable stop。Kernel 边界 approve/deny 现可写入 `KernelStore`，但 native ask 仍无 one-shot consumption、expiry、drift check 或 resume；permission/schema 也不是 command OS sandbox。 | write/edit、command 与 network tool 继续逐层校验 actual arguments；ask 进入 owner-scoped approval store，批准一次性绑定 exact pending call、校验 expiry/drift，并只在同一 active execution中、每个外部 side effect 重验 permit 后 resume，不得隐式重放已结束/漂移调用。 | P1 |
...
| INT-02 | REST API | `partial` | public-core | API 覆盖更广的 daemon/product resources，dispatch/workflow SSE 使用 owner-scoped control。 | Rust `serve` 有 core dispatch/broadcast/tasks/runs/sessions，使用 per-start bearer、loopback Host/Origin enforcement 与 allowlisted views。workflow daemon 有独立 authenticated local workflow API；WP-61 在同一受限 listener 上增加 Linux AgentRun submit/status/output/cancel，WP-66 让同一 allowlisted surface 显式接受 fresh/sessionless Native backend；两者均 deny unknown fields、要求 UUIDv7 且不返回 prompt、credential 或 raw endpoint。WP-80 另以显式 opt-in `GoalReadService` 增加唯一的 `GET /v1/goals/{id}/continuity-next`：component 冻结 `local` owner，单一只读事务生成 allowlisted redacted DTO，request 不能提供 owner；没有 goal list/get、MCP 或任何 mutation。所有 surface 仍冻结为 fixed local single-user scope；bearer 不是 distinct principal，也不提供 hostile same-UID/multi-user isolation | 保持 WP-80 endpoint 只读、原子且 owner-frozen；WP-81 typed observation ingress 与 WP-82 runner 必须使用独立 mutation/execution authority。继续定义 API scope/version/auth ADR，把 authenticated request credential 接入 fixed `OwnerContextFactory`，让其余 control surface 使用 principal-derived scope，并补统一 error schema、pagination/idempotency；private resource 走 optional adapter。 | P2 |
| INT-03 | MCP server | `partial` | public-core | 参考 MCP surface 有 19 个工具，覆盖 task control、workflow、board、check 与 collaboration。 | `vyane-rs:crates/vyane-mcp/src/lib.rs` 保留 6 个 base tools，并提供不持有 daemon credential/discovery 的 object-safe `WorkflowControl` port；`vyane mcp` 由 CLI 注入经认证的 resident-daemon adapter 后提供 9 个工具，新增 bounded self-contained workflow submit/status/idempotent-cancel。caller 只能提供 canonical UUIDv7、source bundle 与 vars，不能提供 owner/controller/token/execution cwd；响应只含 caller id、生命周期和 closed failure code。WP-89 将 SDK 提升到 stable `rmcp` 3.0、MSRV 提升到 Rust 1.88；WP-467 将锁刷新到 `rmcp` 3.3.0，stdio `initialize` 停在 2025-11-25，并通过 `server/discover` 协商 MCP 2026-07-28。继续由 fake-port real-rmcp 与真实 CLI→MCP→authenticated daemon acceptance 覆盖动态 6/9 工具表和调用行为。route/check 仍是 Rust diagnostics，generic success output 仍有 1 MiB 上限 | 继续接统一 durable task control、collaboration/board 与 principal-derived owner context；旧 execution tools 仍需统一逐字段 input budgets，sessions 仍不等于参考多动作 session tool。 | P1 |
| INT-04 | resident daemon | `different` | decision | 广义 daemon 同时承载 worker、scheduler、event、message、memory、goal 与 channel；workflow HTTP 属于另一 process surface。 | Rust daemon 已持有 authenticated local workflow execution，并以 WP-61 与 WP-63 至 WP-66 production-assemble 共享 AgentRun execution/recovery/completion host、fresh/sessionless CLI-harness Process lane 与 fresh/sessionless Native lane；两条 lane 具备 exact backend recovery/cancel 与 message handback，启动先 recovery，restart 不 replay。Process cooperative drain 已有 acceptance；Native daemon-level graceful-shutdown race 也已覆盖 active wire cooperative cancellation、不确定输入保留、deadline 后 restart settlement、no replay/no output 与终态 exact residue cleanup。broker supervisor 已组装但 delivery lane 刻意为空；仍没有 `Remote`、native session resume、distinct-principal wiring、live pause/resume 或 automatic replay。ADR 0004 接受由窄 domain supervisor 组成 authenticated resident host | 继续接入 concrete broker delivery lane 与 `Remote`，统一 principal-derived owner context 和 bounded drain；用 executable acceptance 分别证明每个 domain，不得因已有窄 Process/Native host 或同名 `daemon` 宣称广义 daemon parity。 | P0 |
...
principal-derived owner service/task truth 已落地。WP-61 与 WP-63 至 WP-66 已把窄 Process 与 fresh/sessionless
Native AgentRun lane 接成 authenticated loopback production host：private frozen spool/snapshot、
exact backend recovery/cancel 与 message completion handback；Process cooperative shutdown 与 Native
daemon-level graceful-shutdown race 均已有 executable acceptance，后者覆盖不确定输入保留、deadline
restart settlement、no replay/no output 与 exact terminal residue cleanup。resident broker supervisor
也已生产组装，但 delivery lane set 刻意为空。下一步是 concrete broker delivery adapter、`Remote`、
session-aware native resume、distinct-principal owner context、公开 CLI 与 A2A/Channels adapter。
