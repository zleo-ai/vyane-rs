...
| COL-03 | durable local AgentRun queue/inbox | `partial` | public-core | owner-aware durable queue 支持 delayed delivery、claim/read 与 worker inbox。 | `vyane-agent` 已有 owner-scoped FIFO/generation/lease/recovery/resume/topology/cancel/outbox；`vyane-message` 与 broker 分离持有 message/delivery。WP-59 新增同机 `a2a send/inbox/read`。WP-61 至 WP-66 将 generic execution/recovery/publication host、private frozen spool、exact Linux Process sidecar、fresh/sessionless Native lane 与 message completion handback 生产 assembly 到 authenticated loopback daemon AgentRun submit/status/output/cancel；仅支持 fixed local owner，restart 不 replay。仍无 `Remote`、domain-aware native resume、principal-derived message API 或 Channels adapter | 保持 fencing/recovery/execution/publication ordering、identity reuse、shutdown 与 race tests；下一步把 fixed local bearer 升级为 principal-derived scope，并接 `Remote`/protocol adapter。remote send→local receipt 不得宣称 distributed exactly-once。 | P2 |
...
principal-derived owner service/task truth 已落地。WP-61 至 WP-66 已把窄 Process 与 fresh/sessionless
Native AgentRun lane 接成 authenticated loopback production host：private frozen spool/snapshot、
exact backend recovery/cancel、message completion handback 与 cooperative shutdown。resident broker
supervisor 也已生产组装，但 delivery lane set 刻意为空。下一步是 concrete broker delivery adapter、
`Remote`、session-aware native resume、distinct-principal owner context、公开 CLI 与 A2A/Channels adapter。
