...
| CON-07 | GoalStore、acceptance、quota handoff、pursuit | `partial` | decision | goal lifecycle 包含 acceptance、progress、quota handoff 与自动 pursuit。 | WP-60 新增 `vyane-goal`：owner-scoped 单一 SQLite 真相源、事务型 snapshot/event revision、六态 lifecycle、priority queue、progress、acceptance descriptor，以及本地 `vyane goal` CLI。WP-68 增加 model-free bounded local verifier；WP-69 以 schema v3 保存 owner-scoped、immutable、digest-checked 的完整 bounded verification artifact。WP-70 增加显式 manual pursuer：在 owner/lease fence 下执行 verify→fresh production dispatch segment→reverify，只有全部 acceptance 满足时由 pursuer 完成；manual/no-criteria/segment/failure/timeout 会 pause，外部 lifecycle 变化会 stop。WP-71 以 schema v4 增加 digest-checked pursuit checkpoint，并用 goal revision、checkpoint revision 与 lease generation fence 将 checkpoint/progress 原子提交；显式 resume+new claim 可在 DB 重开后继续 lifetime counters，旧 worker fail closed。WP-72 在显式 target/workdir/sandbox opt-in 下把同一 pursuer 接入常驻 daemon：一次处理一个 local goal，先续接 eligible in-progress work、再原子 claim queued goal；稳定 worker lease 支持 daemon replacement 续接，paused 与 live foreign lease 不会自动 resume 或被窃取，shutdown 保留 running checkpoint。WP-73 以 schema v6 加入分轴 typed continuity policy，并把规范化 quota event 幂等写入可见 handoff plan；无 takeover 明确记录 manual blocker，有 takeover 只生成 declaration，不启动执行。WP-74 以 schema v7 增加 owner-scoped、严格绑定且一次性消费的 takeover approval/dispatch。WP-75 以 schema v8 在 takeover 成功后释放 review step，将 review approval 绑定 exact takeover approval/run evidence，并在 review 成功后只解除 review dependency，primary 仍显式等待 quota reset。仍无 quota ledger reader/runner、quota reset ingestion、primary resume execution、authenticated service/API 或网络/board verifier。 | 保持 ledger、pursuer、continuity state 与 daemon assembly 分层；后续单独接 quota reader/runner、primary resume 和 authenticated authority，不能把 review handback 当成 quota reset 或 primary execution authority。 | P2 |
